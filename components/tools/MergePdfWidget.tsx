    ...
    <div className="flex flex-col gap-4">
      ...
      >
        ...
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)] mt-1">or <span className="text-[var(--brand)]">click to browse</span> · Max 10 files</p>
        </div>
      ...
      {files.length > 0 && (
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] overflow-hidden">
          ...
          <ul>
            {files.map((f, i) => (
              <li key={`${f.name}-${i}`} className="flex items-center gap-3 px-4 py-3 border-b border-[var(--border)] last:border-0">
                <span className="text-[var(--text-subtle)] text-xs font-mono w-5 text-center">{i + 1}</span>
                <span className="text-sm flex-1 truncate">{f.name}</span>
                <span className="text-xs text-[var(--text-muted)]">{formatBytes(f.size)}</span>
                <div className="flex gap-1">
                  <button onClick={() => moveUp(i)} disabled={i === 0} className="text-xs text-[var(--text-muted)] hover:text-[var(--text)] disabled:opacity-30 px-1">↑</button>
                  <button onClick={() => moveDown(i)} disabled={i === files.length - 1} className="text-xs text-[var(--text-muted)] hover:text-[var(--text)] disabled:opacity-30 px-1">↓</button>
      ...
      {files.length === 1 && (
        <p className="text-center text-sm text-[var(--text-muted)]">Add at least one more PDF to merge.</p>
      )}
