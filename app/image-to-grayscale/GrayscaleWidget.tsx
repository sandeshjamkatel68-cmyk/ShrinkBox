    ...
    <div className="w-full flex flex-col gap-4">
      ...
      {(preview || (status === "done" && result)) && (
        <div className="grid grid-cols-2 gap-3">
          {preview && (<div className="rounded-xl border border-border overflow-hidden"><div className="px-3 py-2 text-xs text-muted border-b border-border">Original</div><img src={preview} alt="Original" className="w-full object-cover max-h-40" /></div>)}
          {status === "done" && result?.downloadUrl && (<div className="rounded-xl border border-border overflow-hidden"><div className="px-3 py-2 text-xs text-brand border-b border-border">Black & White</div><img src={result.downloadUrl} alt="Grayscale" className="w-full object-cover max-h-40" /></div>)}
        </div>
      )}
