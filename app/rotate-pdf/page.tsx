  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-16">
      <div className="mb-8 text-center">
        ...
        <p className="text-[var(--text-muted)]">Rotate all pages in a PDF by 90°, 180°, or 270°. Free, instant, no signup.</p>
      </div>
              ...
              {([90, 180, 270] as const).map(a => (
                <button key={a} onClick={() => setAngle(a)}
                  className={["flex-1 rounded-xl border py-2.5 text-sm font-medium transition-all",
                    angle === a ? "border-[var(--brand)] bg-[var(--brand-light)] text-[var(--brand)]"
                    : "border-[var(--border)] text-[var(--text-muted)] hover:border-[var(--brand)]/30"].join(" ")}>
                  {a}°
                </button>
              ))}
            </div>
          </div>
          <button onClick={handleRotate} className="w-full bg-[var(--brand)] hover:bg-[var(--brand-dim)] text-white font-semibold rounded-xl py-3 text-sm transition-colors">
            Rotate PDF {angle}°
          </button>
        </div>
      )}
      ...
      {status === "processing" && (
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-6 py-5 text-center">
          <p className="text-sm font-medium">Rotating...</p>
        </div>
      )}
      ...
      {status === "done" && result && (
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 text-center">
          ...
          <p className="font-semibold mb-1">PDF rotated {angle}°</p>
          <p className="text-sm text-[var(--text-muted)] mb-5">{formatBytes(result.outputSize)}</p>
          <a href={result.downloadUrl} download="rotated.pdf"
