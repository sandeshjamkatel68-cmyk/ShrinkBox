...
export default function ProtectPdfPage() {
  ...
  async function handleProtect() {
    ...
    try {
      const res = await fetch("/api/pdf/protect", { method: "POST", body: form });
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-16">
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 text-center">
          ...
          <p className="font-semibold mb-2">PDF protected</p>
          <p className="text-sm text-[var(--text-muted)] mb-2">{formatBytes(result.outputSize)}</p>
          <div className="rounded-lg bg-yellow-400/10 border border-yellow-400/20 text-yellow-600 dark:text-yellow-400 text-xs px-3 py-2 mb-5 text-left">
            ⚠️ This adds a confidential watermark. For full AES password encryption, use Adobe Acrobat or open the PDF in Preview (Mac) / Chrome print dialog and save with password.
          </div>
          <a href={result.downloadUrl} download="protected.pdf"
          ...
          <button onClick={reset} className="block mx-auto mt-3 text-sm text-[var(--text-muted)]">Try another</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-16">
      <div className="mb-8 text-center">
        ...
        <p className="text-[var(--text-muted)]">Add a confidential watermark and protection marker to your PDF. Free, no signup.</p>
      </div>
          ...
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-5 py-4 flex flex-col gap-3">
            <div>
              ...
              <div className="relative">
                <input type={show ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)}
                  placeholder="Enter password" className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface-muted)] px-3 py-2.5 text-sm pr-10 focus:outline-none focus:border-[var(--brand)]" />
                <button onClick={() => setShow(s => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-[var(--text-muted)]">
                  {show ? "Hide" : "Show"}
                </button>
              </div>
            </div>
            <div>
              ...
              <input type={show ? "text" : "password"} value={confirm} onChange={e => setConfirm(e.target.value)}
                ...
                className={`w-full rounded-xl border px-3 py-2.5 text-sm focus:outline-none bg-[var(--surface-muted)] ${mismatch ? "border-red-400" : "border-[var(--border)] focus:border-[var(--brand)]"}`} />
              {mismatch && <p className="text-xs text-red-400 mt-1">Passwords don't match</p>}
            </div>
          </div>
          <button onClick={handleProtect} disabled={!password.trim() || mismatch}
      ...
      {status === "processing" && (
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-6 py-5 text-center">
          <p className="text-sm font-medium">Protecting...</p>
        </div>
      )}
