      ...
      {file && status === "idle" && (
        <div className="flex flex-col gap-4">
          <div className="rounded-2xl border border-border bg-surface px-5 py-4 flex flex-col gap-3">
            <div><label className="text-sm font-medium block mb-1.5">Password</label><div className="relative"><input type={show ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter password" className="w-full rounded-xl border border-border bg-surface-muted px-3 py-2.5 text-sm pr-10 focus:outline-none focus:border-brand" /><button onClick={() => setShow(s => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted">{show ? "Hide" : "Show"}</button></div></div>
            <div><label className="text-sm font-medium block mb-1.5">Confirm password</label><input type={show ? "text" : "password"} value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="Re-enter password" className={`w-full rounded-xl border px-3 py-2.5 text-sm focus:outline-none bg-surface-muted ${mismatch ? "border-red-400" : "border-border focus:border-brand"}`} />{mismatch && <p className="text-xs text-red-400 mt-1">Passwords don&apos;t match</p>}</div>
          </div>
          <button onClick={handleProtect} disabled={!password.trim() || mismatch} className="w-full bg-brand hover:bg-[var(--brand-dim)] disabled:opacity-40 text-white font-semibold rounded-xl py-3 text-sm transition-colors">Protect PDF</button>
        </div>
      )}
      {status === "processing" && (<div className="rounded-2xl border border-border bg-surface px-6 py-5 text-center"><p className="text-sm font-medium">Protecting...</p></div>)}
