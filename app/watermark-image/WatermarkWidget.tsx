      ...
      {file && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
          <div className="bg-surface border border-border rounded-2xl p-6 space-y-4">
            <div>
              <label className="block text-xs font-bold text-subtle uppercase tracking-wider mb-2">Watermark Text</label>
              <input 
                type="text" 
                value={text} 
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-subtle uppercase tracking-wider mb-2">Color</label>
                <input 
                  type="color" 
                  value={color} 
              ...
              <div>
                <label className="block text-xs font-bold text-subtle uppercase tracking-wider mb-2">Size (%)</label>
                <input 
                  type="number" 
                  value={size} 
                  onChange={(e) => setSize(Number(e.target.value))}
                  className="w-full h-11 px-4 rounded-xl border border-border bg-surface-muted text-sm font-medium"
                />
              </div>
            </div>

            <div>
              <label className="block text-xs font-bold text-subtle uppercase tracking-wider mb-2">Opacity</label>
              <input 
                type="range" 
                min="0.1" max="1" step="0.1" 
                value={opacity} 
                onChange={(e) => setOpacity(Number(e.target.value))}
                className="w-full h-2 rounded-lg bg-surface-muted appearance-none cursor-pointer accent-brand mt-2"
              />
            </div>

            <div>
              <label className="block text-xs font-bold text-subtle uppercase tracking-wider mb-2">Position</label>
                ...
                {(["top-left", "top-right", "center", "bottom-left", "bottom-right"] as Position[]).map((pos) => (
                  <button 
                    ...
                    className={`h-9 px-2 rounded-lg text-[10px] font-bold border transition-all 
                      ${position === pos ? 'bg-brand text-white border-brand shadow-sm' : 'bg-transparent text-muted border-border hover:border-[var(--brand-muted)]'}`}
                  >
                    {pos.replace("-", " ")}
                  </button>
                ))}
          ...
          <div className="bg-surface-muted border border-border rounded-2xl p-4 flex flex-col items-center justify-center min-h-[300px]">
            {previewUrl ? (
              ...
              <p className="text-xs font-medium text-muted">Live preview will appear here</p>
            )}
          </div>
        </div>
      )}
