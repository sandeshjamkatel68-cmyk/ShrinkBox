  ...
  return (
    <div className="mt-8 space-y-6">
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 space-y-4">
        <div>
          <label className="block text-xs font-bold text-[var(--text-subtle)] uppercase tracking-wider mb-2">Paste Base64 String</label>
          <textarea 
            value={input}
            onChange={(e) => setInput(e.target.value)}
            className="w-full h-32 p-4 rounded-xl border border-[var(--border)] bg-[var(--surface-muted)] text-[10px] font-mono leading-normal focus:outline-none focus:border-[var(--brand)] transition-colors resize-none"
        ...
        {previewUrl && !error && (
          <div className="pt-4 border-t border-[var(--border)] space-y-4 flex flex-col items-center animate-in fade-in duration-500">
            <div className="bg-[var(--surface-muted)] border border-[var(--border)] rounded-xl p-4 flex items-center justify-center min-h-[200px] w-full">
              <img 
                src={previewUrl} 
                alt="Base64 preview" 
