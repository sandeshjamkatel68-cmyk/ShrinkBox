...
export default function ImagesToPdfWidget() {
  ...
  function addFiles(list: FileList | null) { if (!list) return; const valid = Array.from(list).filter(f => /\.(jpe?g|png|webp)$/i.test(f.name)); setFiles(prev => [...prev, ...valid].slice(0, 20)); }
  function removeFile(i: number) { setFiles(f => f.filter((_, idx) => idx !== i)); }

  async function handleConvert() {
    if (!files.length) return; setStatus("processing"); setError(null);
    const form = new FormData(); files.forEach(f => form.append("files", f));
    try { const res = await fetch("/api/convert/images-to-pdf", { method: "POST", body: form }); const data = await res.json(); if (!data.success) { setError(data.error); setStatus("error"); return; } setResult(data); setStatus("done"); } catch { setError("Network error."); setStatus("error"); }
  }
