  ...
  return (
    <div className="mt-8 space-y-6">
      ...
      >
        ...
        <p className="text-xs text-[var(--text-muted)]">Clear screenshots or scans work best</p>
      </div>
      ...
      {text && (
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 space-y-4 animate-in fade-in duration-500">
          <div className="flex items-center justify-between pb-2 border-b border-[var(--border)]">
            <h4 className="text-xs font-bold text-[var(--text-subtle)] uppercase tracking-wider">Extracted Text</h4>
            <button 
          ...
          <div className="max-h-[300px] overflow-y-auto pr-2 rounded-lg py-1">
            <p className="text-sm text-[var(--text)] leading-relaxed whitespace-pre-wrap font-sans selection:bg-[var(--brand-light)]">
              {text}
            </p>
          </div>
          <button 
            onClick={() => { setText(null); setFile(null); }}
            className="w-full text-xs font-medium text-[var(--text-muted)] hover:text-[var(--text)] pt-2"
          >
            Start Over
          </button>
        </div>
