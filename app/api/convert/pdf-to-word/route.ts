...
import { pdfToWord } from "@/lib/tools/pdfToWord";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await pdfToWord(buffer, file.name);
...
function err(error: string, status: number) {
  return NextResponse.json({ success: false, outputSize: 0, pageCount: 0, processingTimeMs: 0, outputFileName: "", error }, { status });
}
