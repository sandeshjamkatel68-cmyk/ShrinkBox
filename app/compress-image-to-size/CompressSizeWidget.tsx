        ...
        <div className="bg-surface border border-border rounded-2xl p-6 space-y-4">
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1">
              <label className="block text-xs font-bold text-subtle uppercase tracking-wider mb-2">Target Size (KB)</label>
              <div className="relative">
                <input 
                  type="number" 
                  value={targetSizeKb}
                  onChange={(e) => setTargetSizeKb(Math.max(1, parseInt(e.target.value) || 0))}
                  className="w-full h-11 px-4 pr-12 rounded-xl border border-border bg-surface-muted text-sm font-bold focus:outline-none focus:border-brand transition-colors"
                />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-muted">KB</span>
              </div>
            </div>
            <button 
          ...
          {result && (
            <div className="pt-4 border-t border-border fade-in">
              <div className="flex items-center justify-between p-4 rounded-xl bg-[var(--brand-light)]/40 border border-[var(--brand-muted)]">
                <div>
                  <p className="text-xs font-bold text-brand uppercase tracking-tight">Optimized Size</p>
                  <p className="text-lg font-bold text-foreground">{(result.size / 1024).toFixed(1)} KB</p>
                </div>
                <button 
