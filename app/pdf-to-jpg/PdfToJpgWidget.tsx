...
export default function PdfToJpgWidget() {
  ...
  async function handleConvert() {
    ...
    try { const res = await fetch("/api/convert/pdf-to-jpg", { method: "POST", body: form }); const data = await res.json(); if (!data.success) { setError(data.error); setStatus("error"); return; } setPages(data.pages); if (data.error) setWarning(data.error); setStatus("done"); } catch { setError("Network error."); setStatus("error"); }
  }
  function downloadAll() { pages.forEach((p, i) => setTimeout(() => { const a = document.createElement("a"); a.href = p.downloadUrl; a.download = p.fileName; a.click(); }, i * 300)); }
