...
export default function AddPageNumbersPage() {
  ...
  const btnClass = (active: boolean) =>
    `rounded-xl border py-2 px-3 text-sm font-medium transition-all ${active
  ...
  async function handleAdd() {
    ...
    try {
      const res = await fetch("/api/pdf/page-numbers", { method: "POST", body: form });
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-16">
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 text-center">
          ...
          <p className="font-semibold mb-1">Page numbers added — {result.pageCount} pages</p>
          <p className="text-sm text-[var(--text-muted)] mb-5">{formatBytes(result.outputSize)}</p>
          <a href={result.downloadUrl} download="numbered.pdf"
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-16">
      <div className="mb-8 text-center">
        ...
        <h1 className="text-3xl font-bold mb-2">Add Page Numbers to PDF</h1>
        <p className="text-[var(--text-muted)]">Automatically number every page of your PDF. Choose position, format and style.</p>
      </div>
          ...
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-5 py-4 flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {([
                  ...
                  <button key={v} onClick={() => setPosition(v)} className={btnClass(position === v)}>{l}</button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <button onClick={() => setFormat("number")} className={btnClass(format === "number")}>1, 2, 3…</button>
                <button onClick={() => setFormat("page-of-total")} className={btnClass(format === "page-of-total")}>1 / 10, 2 / 10…</button>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <button onClick={() => setColor("black")} className={btnClass(color === "black")}>Black</button>
                <button onClick={() => setColor("gray")} className={btnClass(color === "gray")}>Gray</button>
              </div>
            </div>

            {/* Start from */}
            <div>
              <label className="text-sm font-medium block mb-1.5">Start from page number</label>
              <input type="number" value={startFrom} min={1} max={999}
                onChange={e => setStartFrom(Math.max(1, Number(e.target.value)))}
                className="w-24 rounded-xl border border-[var(--border)] bg-[var(--surface-muted)] px-3 py-2 text-sm focus:outline-none focus:border-[var(--brand)]" />
            </div>
          </div>
      ...
      {status === "processing" && (
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-6 py-5 text-center">
          <p className="text-sm font-medium">Adding page numbers...</p>
        </div>
      )}
